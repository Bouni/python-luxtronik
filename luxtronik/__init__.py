        ...
        for i in range(0, len(data)):
            ...
            if isinstance(l["conversion"], dict):
                self._data[target][i] = {
                    "id": l["id"],
                    "unit": "info",
                    "value": l["conversion"].get(raw, raw),
                }
            else:
                if l["conversion"] == "celsius":
                    ...
                    self._data[target][i] = {
                        "id": l["id"],
                        "unit": "ipaddress",
                        "value": f"{raw >> 24 & 0xFF}.{raw >> 16 & 0xFF}.{raw >> 8 & 0xFF}.{raw & 0xFF}",
                    }
                elif l["conversion"] == "timestamp":
                    self._data[target][i] = {
                        "id": l["id"],
                        "unit": "datetime",
                        "value": datetime.datetime.fromtimestamp(raw),
                    }
                    ...
                    self._data[target][i] = {
                        "id": l["id"],
                        "unit": "version",
                        "value": "".join([chr(c) for c in data[i : i + 9]]).strip(
                            "\x00"
                        ),
                    }
                else:
